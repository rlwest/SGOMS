...
################ methods #######################
    ...
    def cut_wire(b_method='method:cut_wire target:?target state:start'):  # target is the chunk to be altered
        motor.cut_wire(target, "cut")
        b_method.set('method:add target:?target state:running')
        print 'target object = ', target

    def done_cut(b_method='method:?method target:?target state:running',
