...
class MemoryPlayer(ACTR):
    ...
    def start_recall(goal='play rps',choice='waiting:True',imaginal='?last'):
        goal.set('recall')
        memory.request('history ?last ?')
    ...
    def check_response(goal='check response',choice='opponent:?choice visible:True',imaginal='?last'):
        memory.add('history ?last ?choice')
        retrieval.clear()
        imaginal.set('?choice')
        goal.set('play rps')
