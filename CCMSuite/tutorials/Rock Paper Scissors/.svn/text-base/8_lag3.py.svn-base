...
class MemoryPlayer(ACTR):
    ...
    def start_recall(goal='play rps',choice='waiting:True',imaginal='?last ?last2 ?last3'):
        goal.set('recall')
        memory.request('history ?last ?last2 ?last3')
    ...
    def check_response(goal='check response',choice='opponent:?choice visible:True',imaginal='?last ?last2 ?last3'):
        memory.add('history ?last ?last2 ?last3 ?choice')
        retrieval.clear()
        imaginal.set('?last2 ?last3 ?choice')
        goal.set('play rps')
