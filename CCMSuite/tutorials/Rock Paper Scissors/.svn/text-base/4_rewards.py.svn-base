...
class RockPlayer(ACTR):
    ...
    def check_reward_win(goal='check reward',choice='state:win'):
        self.success()
        goal.set('play rps')
    def check_reward_loss(goal='check reward',choice='state:loss'):
        self.failure()
        goal.set('play rps')
    def check_reward_tie(goal='check reward',choice='state:tie'):
        self.failure()
        goal.set('play rps')
...
env.model1=RockPlayer()
env.model1.choice=env.choice1

env.model2=RockPlayer()
